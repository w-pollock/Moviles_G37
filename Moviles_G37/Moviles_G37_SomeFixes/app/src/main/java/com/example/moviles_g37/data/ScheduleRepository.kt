...
/**
 * Per-user weekly schedule stored under users/{uid}/schedule.
 * Users enter their schedule from SettingsScreen → Schedule Editor; the app
 * never invents classes for the user.
 *
 * If the user has nothing saved, getSchedule() returns an empty list so the
 * UI can show an empty state inviting them to add classes.
 */
class ScheduleRepository {
    ...
    /** Re-map Sunday so that Monday comes first when sorting. */
