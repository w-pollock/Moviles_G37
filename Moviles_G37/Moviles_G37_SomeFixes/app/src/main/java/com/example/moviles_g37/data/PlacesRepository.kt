...
/**
 * Reads places from Firestore "places" collection. If the collection is empty
 * on the first launch, seeds it with the SenecaPlaces catalogue so the Firebase
 * console becomes the single source of truth going forward (no mock data in UI).
 */
class PlacesRepository {

    private val db = FirebaseFirestore.getInstance()

    /** Loads all places. Falls back to the seed catalogue on error. */
    ...
    /**
     * One-shot seed. If the "places" collection has any document, returns
     * immediately. Otherwise pushes all SenecaPlaces entries to Firestore.
     */
